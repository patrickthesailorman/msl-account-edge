...
public class AccountEdgeServiceTest {
    ...
    public void testGetMyLibrary()
        throws Exception {
        Mockito.when(libraryService.get(cassandraAccountService, tc.USER_ID.toString())).thenReturn(new MyLibrary());
        PowerMock.replayAll();
    ...
    public void testAddToLibrary() {
        ...
        Mockito.verify(libraryService, atLeastOnce()).add(cassandraAccountService, tc.ARTIST_UUID.toString(),
    ...
    public void testRemoveFromLibrary() {
        PowerMock.replayAll();
        AccountEdgeService accountEdgeService = new AccountEdgeService(libraryService);
        accountEdgeService.removeFromLibrary(tc.ARTIST_UUID.toString(), tc.FAVORITES_TIMESTAMP.toString(),
                                             tc.USER_ID.toString(), "Artist");
        Mockito.verify(libraryService, atLeastOnce()).remove(cassandraAccountService, tc.ARTIST_UUID.toString(),
                                                             tc.FAVORITES_TIMESTAMP.toString(), tc.USER_ID.toString(),
                                                             "Artist");
    }
