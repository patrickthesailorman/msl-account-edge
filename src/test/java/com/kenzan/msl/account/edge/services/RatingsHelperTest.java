...
public class RatingsHelperTest {
  ...
  private Integer average = (int) (tc.AVERAGE_RATINGS_DTO.getNumRating() / tc.AVERAGE_RATINGS_DTO
      .getSumRating());
  ...
  public void testProcessAlbumRatings() {
    expect(cassandraRatingsService.getAverageRating(tc.ALBUM_UUID, "Album")).andReturn(
        Observable.just(Optional.of(tc.AVERAGE_RATINGS_DTO)));

    expect(cassandraRatingsService.getUserRating(tc.USER_ID, "Album", tc.ALBUM_UUID)).andReturn(
        Observable.just(Optional.of(tc.USER_RATINGS_DTO)));
    ...
    Integer average =
        (int) (tc.AVERAGE_RATINGS_DTO.getNumRating() / tc.AVERAGE_RATINGS_DTO.getSumRating());
  ...
  @Test
  public void testProcessArtistRatings() throws Exception {
    expect(cassandraRatingsService.getAverageRating(tc.ARTIST_UUID, "Artist")).andReturn(
        Observable.just(Optional.of(tc.AVERAGE_RATINGS_DTO)));

    expect(cassandraRatingsService.getUserRating(tc.USER_ID, "Artist", tc.ARTIST_UUID)).andReturn(
        Observable.just(Optional.of(tc.USER_RATINGS_DTO)));
  ...
  @Test
  public void testProcessSongRatings() throws Exception {
    expect(cassandraRatingsService.getAverageRating(tc.SONG_UUID, "Song")).andReturn(
        Observable.just(Optional.of(tc.AVERAGE_RATINGS_DTO)));

    expect(cassandraRatingsService.getUserRating(tc.USER_ID, "Song", tc.SONG_UUID)).andReturn(
        Observable.just(Optional.of(tc.USER_RATINGS_DTO)));
